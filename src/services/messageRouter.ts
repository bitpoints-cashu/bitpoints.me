...
export class MessageRouter {
  ...
  async sendToken(
    ...
    if (isReachable) {
      console.log(
        `📡 Routing token via Bluetooth mesh to ${recipientPeerID.substring(
          0,
          8
        )}...`
      );
    ...
    if (this.canSendViaNostr(recipientPeerID)) {
      console.log(
        `📨 Routing token via Nostr to ${recipientPeerID.substring(0, 8)}...`
      );
    ...
    console.log(
      `📦 Queuing token for ${recipientPeerID.substring(
        0,
        8
      )}... (no Bluetooth, no Nostr mapping)`
    );
  ...
  async flushOutbox(peerID: string) {
    ...
    console.log(
      `📬 Flushing ${queued.length} queued tokens for ${peerID.substring(
        0,
        8
      )}...`
    );
  ...
  cleanupOutbox() {
    ...
    if (this.outbox.length < before) {
      console.log(
        `🧹 Cleaned up ${before - this.outbox.length} old queued tokens`
      );
    }
  }
}
