...
export function useWatchIntegration() {
  const connectedWatches = useLocalStorage<WatchConnection[]>(
    "bitpoints.connectedWatches",
    []
  );
  ...
  function addWatch(watch: WatchConnection) {
    const existingIndex = connectedWatches.value.findIndex(
      (w) => w.id === watch.id
    );
    if (existingIndex >= 0) {
   ...
   */
  async function sendTokenToWatch(
    watchId: string,
    tokenString: string,
    memo?: string
  ): Promise<boolean> {
   ...
   */
  async function sendTokenToAllWatches(
    tokenString: string,
    memo?: string
