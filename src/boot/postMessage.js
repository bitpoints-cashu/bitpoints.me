...
export default ({ app }) => {
  ...
  const getAllowedOrigins = () => {
    ...
    return ['http://localhost:3000', 'http://localhost:5173', 'http://localhost:8080', 'https://bitpoints.me']
  }
  ...
  const handlePostMessage = (event) => {
    ...
    if (!allowedOrigins.includes(event.origin)) {
      console.warn('PostMessage: Received message from unauthorized origin:', event.origin)
      return
    }
