    ...
    <q-card style="min-width: 400px">
      ...
      <q-card-section>
        <div class="text-body1 q-mb-md">
          To keep Bitpoints running 24/7, we need to disable battery
...
async function onAllow() {
  ...
  try {
    ...
    show.value = false;
    notifySuccess(
      "Battery optimization settings opened. Please allow Bitpoints to run in background."
    );
  } catch (error) {
    console.error("Failed to open battery settings:", error);
    notifyError(
      "Failed to open battery settings. Please enable manually in Android Settings."
    );
  } finally {
    loading.value = false;
  }
