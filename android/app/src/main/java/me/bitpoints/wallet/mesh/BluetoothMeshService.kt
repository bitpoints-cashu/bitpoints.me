...
import me.bitpoints.wallet.crypto.EncryptionService
import me.bitpoints.wallet.model.BitchatMessage
import me.bitpoints.wallet.protocol.MessagePadding
import me.bitpoints.wallet.model.RoutedPacket
import me.bitpoints.wallet.model.IdentityAnnouncement
import me.bitpoints.wallet.protocol.BitchatPacket
import me.bitpoints.wallet.protocol.MessageType
import me.bitpoints.wallet.protocol.SpecialRecipients
import me.bitpoints.wallet.model.RequestSyncPacket
import me.bitpoints.wallet.sync.GossipSyncManager
import me.bitpoints.wallet.util.toHexString
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    me.bitpoints.wallet.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    me.bitpoints.wallet.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    me.bitpoints.wallet.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
    ...
    private fun setupDelegates() {
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    me.bitpoints.wallet.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        me.bitpoints.wallet.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun handleCustomPacket(packet: BitchatPacket, fromPeerID: String?, relayAddress: String?) {
                // Forward custom packets to the mesh delegate (e.g., for ecash)
                Log.d(TAG, "Received custom packet type 0x${packet.type.toString(16)} from ${fromPeerID}, forwarding to delegate")
                delegate?.didReceiveCustomPacket(packet, fromPeerID, relayAddress)
            }
        }
        ...
        connectionManager.delegate = object : BluetoothConnectionManagerDelegate {
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendFileBroadcast(file: me.bitpoints.wallet.model.BitchatFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: me.bitpoints.wallet.model.BitchatFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = me.bitpoints.wallet.model.NoisePayload(
                            type = me.bitpoints.wallet.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            // Note: Geohash/MessageRouter not implemented in cashu.me
            // This is specific to bitchat messaging features

            try {
                // Create read receipt payload using NoisePayloadType exactly like iOS
                val readReceiptPayload = me.bitpoints.wallet.model.NoisePayload(
                    type = me.bitpoints.wallet.model.NoisePayloadType.READ_RECEIPT,
     ...
     */
    fun getSessionState(peerID: String): me.bitpoints.wallet.noise.NoiseSession.NoiseSessionState {
...
interface BluetoothMeshDelegate {
    ...
    fun didReceiveCustomPacket(packet: BitchatPacket, fromPeerID: String?, relayAddress: String?) {
        // Default no-op implementation for custom packets
    }
