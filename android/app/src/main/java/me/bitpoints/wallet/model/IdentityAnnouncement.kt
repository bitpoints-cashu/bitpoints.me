...
import me.bitpoints.wallet.util.*
...
data class IdentityAnnouncement(
    ...
    fun encode(): ByteArray? {
        ...
        if (features != 0) {
            ...
            // Trim leading zeros for minimal encoding
            var start = 0
            while (start < 3 && featBytes[start] == 0.toByte()) start++
            val trimmed = featBytes.sliceArray(start until 4)
            
            result.add(TLVType.FEATURES.value.toByte())
            result.add(trimmed.size.toByte())
            result.addAll(trimmed.toList())
        }
