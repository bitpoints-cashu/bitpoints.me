...
import me.bitpoints.wallet.model.BitchatMessageType
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
            ...
            if (noisePayload != null) {
                ...
                when (noisePayload.type) {
                    ...
                    if (file != null) {
                        ...
                        val savedPath = me.bitpoints.wallet.util.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = me.bitpoints.wallet.model.NoisePayload(
                type = me.bitpoints.wallet.model.NoisePayloadType.DELIVERED,
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = me.bitpoints.wallet.protocol.MessageType.fromValue(packet.type) == me.bitpoints.wallet.protocol.MessageType.FILE_TRANSFER
            val file = me.bitpoints.wallet.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
    ...
    private suspend fun handlePrivateMessage(packet: BitchatPacket, peerID: String) {
        try {
            ...
            val messageType = me.bitpoints.wallet.protocol.MessageType.fromValue(packet.type)
            if (messageType != me.bitpoints.wallet.protocol.MessageType.MESSAGE) {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                me.bitpoints.wallet.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    me.bitpoints.wallet.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                ...
                val sys = me.bitpoints.wallet.model.BitchatMessage(
                    sender = "system",
