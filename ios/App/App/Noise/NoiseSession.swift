...
class NoiseSession {
    ...
    // Thread safety
    private let sessionQueue = DispatchQueue(label: "me.bitpoints.wallet.noise.session", attributes: .concurrent)
    ...
    func processHandshakeMessage(_ message: Data) throws -> Data? {
        return try sessionQueue.sync(flags: .barrier) {
            print("NoiseSession[\(peerID)]: Processing handshake message, current state: \(state), role: \(role)")
            ...
            if state == .uninitialized && role == .responder {
                ...
                state = .handshaking
                print("NoiseSession[\(peerID)]: Initialized handshake state for responder")
            }
            ...
            print("NoiseSession[\(peerID)]: Read handshake message, checking if complete")
            ...
            if handshake.isHandshakeComplete() {
                ...
                print("NoiseSession[\(peerID)]: Handshake complete (no response needed), transitioning to established")
                ...
                print("NoiseSession[\(peerID)]: Generated handshake response of size \(response.count)")
                ...
                if handshake.isHandshakeComplete() {
                    ...
                    print("NoiseSession[\(peerID)]: Handshake complete after writing response, transitioning to established")
                }
    ...
    func reset() {
        sessionQueue.sync(flags: .barrier) {
            ...
            if wasEstablished {
                print("NoiseSession[\(peerID)]: Session expired")
            }
        }
    }
}
